...
export default {
  ...
  methods: {
    ...
    async removeUserById(id) {
      //弹框询问用户是否删除
      const confirmResult = await this.$confirm('此操作将永久删除该用户,是否继续?', '提示', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' }).catch((err) => err)
      //如果用户确认删除,则返回值为字符串confirm
      //如果用户点击取消,通过catch捕获返回值cancel
